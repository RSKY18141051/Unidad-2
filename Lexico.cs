...
namespace sintaxis3
{
    class Lexico : Token, IDisposable
    {
        ...
        public Lexico(string nombre)
        {
            ...
            {
                throw new Exception(String.Format("El archivo {0} no es un archivo cpp.", name));
            }
            ...
            if (File.Exists(nombre))
            {
                ...
                bitacora.AutoFlush = true;
                DateTime fechaActual = DateTime.Now;
                bitacora.WriteLine("Archivo: " + name);
                bitacora.WriteLine("Directorio: " + nombreDir);
                bitacora.WriteLine("Fecha: " + fechaActual);
            }
            else
            {
                string mensaje = String.Format("El archivo {0} no existe.", name);
